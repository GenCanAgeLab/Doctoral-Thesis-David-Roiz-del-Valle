...
getcircseq <- function(circgr, exongrs,fafile, returnexns=F,fixcoords=F){
  ...
  if (length(exin) == 0){
    warning("circRNA does not match exon junctions.Returning genomic sequence")
    s <- unlist(getSeq(fafile, unstrand(circgr)))
  ...
  warning(sprintf("sdif = %s, edif = %s", sdif,edif))
  if (sdif != 0 | edif !=0){
...
(s <- getcircseq(mycircs[mycircs$circID == 'cRNA_0832'],myex,fa))
(o <- find_orfs(s))
(pcr <- pcr_temp(s))
